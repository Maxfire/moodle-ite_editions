...
$PAGE->set_url('/mod/mgm/admin/admin_mgm.php');
...
if ($data = $mform->get_data ( false )) {
	if (! empty ( $data->cancel )) {
		...
		if ($save && $data->action == 'loadchistory') {
			...
			if ($handle) {
				...
				while ( ($reg = fgetcsv ( $handle, 0, "," )) !== FALSE ) {
					...
					$n++;
					$format = '%06d -- %s';
					echo sprintf($format, $n, $dev[1]);
				}
				
				echo '<br/>Todos los registros procesados<br>';
				fclose ( $handle );
				@unlink ( $filename );
			} else {
				echo 'No se puede abrir el fichero:' . $file;
			}
		} else if ($save && $data->action == 'loadcolegios') {
			$handle = fopen ( $filename, "r" );
			if ($handle) {
				$fields = fgetcsv ( $handle, 0, "," );
				$n = 0;
				while ( ($reg = fgetcsv ( $handle, 0, "," )) !== FALSE ) {
					if (count ( $reg ) >= 13) {
						...
						$res ='<br />Registro incorrecto';
					}
					$n++;
					$format = '%06d -- %s';
					echo sprintf($format, $n, $res);					
				}
				echo '<br/>Todos los registros procesados<br>';
				fclose ( $handle );
				@unlink ( $filename );

...
if ($action=='1'){
  ...
  if (! isset($SESSION->bulk_filters['edicionid']) || $SESSION->bulk_filters['edicionid']<=0){
   	print_error('Debe seleccionar una edición!', $return);
  }
	...
	if ($msgform->is_cancelled()) {
	    ...
	    $in = implode(',', $SESSION->bulk_users);
	    $userlist = $DB->get_records_select_menu('user', "id IN ($in)", 'fullname', 'id,'.$DB->sql_fullname().' AS fullname');
	    ...
	    echo $OUTPUT->box_start('boxwidthnarrow boxaligncenter generalbox');
	...
	echo $OUTPUT->header();	
	$msgform->display();
	echo '<div><a><b>Comonides disponibles para el alumno:</b> #nombre, #apellidos, #email, #cc, #usuario </a></div>';
	echo '<div><a><b>Comonides disponibles para el centro:</b> #dgenerica, #despecifica, #cp, #direccion, #localidad, #provincia, #pais, #telefono  </a></div>';
	echo '<div><a><b>Comonides disponibles para el curso:</b>  #curso  </a></div>';
	echo $OUTPUT->footer();
	//admin_externalpage_print_footer();
}else if ($action=='2'){

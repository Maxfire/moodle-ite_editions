...
class plugin_courses extends plugin_base{
	...
	function print_filter(&$mform){
		...
		if($this->report->type != 'sql'){
			...
			if ($filter_editions != 0){
				$sql="select courseid from ".$CFG->prefix."edicion_course where edicionid=".$filter_editions;
				$courselist= array_keys(get_records_sql($sql));
			}else
				$courselist = array_keys(get_records('course'));
		}
